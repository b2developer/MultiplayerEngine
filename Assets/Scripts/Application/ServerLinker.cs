...
public class ServerLinker
{
    ...
    //send the initial request for hosting
    public void SendHostRequest()
    {
        BitStream stream = new BitStream(5);
        stream.WriteInt((int)Settings.ELinkingStateType.HOST, 3);
    ...
    public void Update(float deltaTime)
    {
        ...
        if (timer > Settings.SERVER_LINKER_TIMEOUT)
        {
            timer -= Settings.SERVER_LINKER_TIMEOUT;
        ...
        lock (messageBuffer)
        {
            foreach (RecievedMessage message in messageBuffer)
            {
                if (message.type == Settings.EMessageType.UNRELIABLE)
                {
                    ...
                    if (state == ELinkerState.WAITING && stateType == Settings.ELinkingStateType.LOCAL_IP_DETECTED)
                    {
                    ...
                    else if (state == ELinkerState.WAITING && stateType == Settings.ELinkingStateType.LINK)
                    {
